...
/* Disabled in favor of promised-based version
export function moduleStatus(ipc, ident, cb) {
  checkProcPath();
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      files.forEach(async (file, index, array) => {
        clog.debug(file);
        var moduleName = file.replace('.pid', '');
        await moduleStatus(ipc, moduleName)
          .then((result) => {
            clog.debug(result);
            runningModules.push(result);
            // eslint-disable-next-line prettier/prettier
            if (index === (array.length - 1)) return resolve(runningModules);
            return;
          })
          .catch((err) => {
            return reject(err);
