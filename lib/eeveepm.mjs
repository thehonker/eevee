...
export function moduleStartPromise(ipc, ident, args) {
  ...
  return new Promise((resolve, reject) => {
    moduleStatus(ipc, ident)
      .then((result) => {
          ...
          case 'running':
            ...
            switch (result.pidFileStatus) {
              case 'valid':
                return reject(
                  new Error({
                    message: 'Module already running (valid pid file)',
                    code: 'E_RUNNING_VALID',
                  }),
                );
              case 'stale':
                return reject(
                  new Error({
                    message: 'Module already running (invalid pid file)',
                    code: 'E_RUNNING_STALE',
                  }),
                );
              default:
                return reject(
          ...
          case 'unresponsive':
            ...
            switch (result.pidFileStatus) {
              case 'stale':
                return reject(
                  new Error({
                    message: 'Module might be running but did not respond to ping (invalid pid file)',
                    code: 'E_UNRESPONSIVE_STALE',
                  }),
                );
              default:
                break;
        ...
        return reject(new Error('We should not have reached this point.'));
      })
      .catch((err) => {
        return reject(err);
      });
